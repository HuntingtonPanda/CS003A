...

{
    ...
    //makes a red dot where cursor is
...
void animate::Draw(){
    ...
    //Draws all my buttons and text for buttns
    button.Draw(window);

    //Writes the history out
    button.writeHistory(window);

    //makes a dot where cursor is
    if (mouseIn){
...
void animate::update(){
    ...
    //system.Step(command);    <<<<<<<<< RIGHT HERE >>>>>>>>>
    
    //Calls to graph manipulator functions
    system.Pan(command);
    system.Zoom(command);
    //system.calc_points();
   ...
   while (window.pollEvent(event))//or waitEvent
       ...
       // check the type of the event...
           ...
           case sf::Event::Closed:
            ...
            case sf::Event::TextEntered:    //Detects if text is being typed
                button.typingText(event);   //Sends detected text to button
                break;                      //Function recieves text and draws it and records it
           // key pressed
        ...
        //        break;
           case sf::Event::MouseButtonPressed:
                ...
                // cout << "mouseX: " << mouseX << "\nmouseY: " << mouseY << endl;
                // cout << "Domain [ " << (30 + WORK_PANEL) << ", " << 30 + WORK_PANEL + SIDE_BAR/4 - 40 << "]\n";
                // cout << "Range [ " << (SCREEN_HEIGHT - 500) << ", " << SCREEN_HEIGHT - 500 + 60 << "]\n";
                   ...
                   else{ //Sees if left click has been pressed
                    if(button.processButton(window)){ //Sees whcih button has been pressed if its enter then do inside
                        system.calc_points(button.getInput()); //Regraphs the points
                        button.writeHistory(window);           //Adds to history and redraws it
                    }
