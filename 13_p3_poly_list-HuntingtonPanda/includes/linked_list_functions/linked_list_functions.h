...
template <typename ITEM_TYPE>
node<ITEM_TYPE>* 
_insert_after(node<ITEM_TYPE>*& head,
                                node<ITEM_TYPE> *after_this,
                                ITEM_TYPE insert_this){
    if(!head){
    ...
    else
    {
        ...
        node<ITEM_TYPE>* temp = after_this->_next;
...
node<ITEM_TYPE>* _insert_sorted(node<ITEM_TYPE>* &head,       //insert
    ...
    // while(walker){
    //     if((ascending && item < walker->_item) || (!ascending && item > walker->_item)){
    //         cout << "Item: " << item << " List prior: " << walker->_item;
    ...
    //     walker = walker->_next;
    // }  
    if(!_where_this_goes(head, item, ascending)){
...
node<ITEM_TYPE>* _insert_sorted_and_add(node<ITEM_TYPE>* &head,
    ...
    // if(temp && item == temp->_item){
    //     temp->_item += item;
    //     return temp;
    // }

    return _insert_sorted<ITEM_TYPE>(head, item, ascending);
}
//node after which this item goes order: 0 ascending
template <typename ITEM_TYPE>
node<ITEM_TYPE>* _where_this_goes(node<ITEM_TYPE>* head,
    ...
    if(ascending){
        while(walker){
            if(item < walker->_item){
                ...
                return _previous_node(head, walker);
            }
            walker = walker->_next;
        }
